...
To test different versions of Kotlin (backed by the `:compiler-compat` system, see its README for more details), set the `kotlin.compiler.plugin.devkit.defaultTestVersion` property to the Kotlin version you want to test. This is automatically used by all the tests in `:compiler`, `:compiler-tests`, and `:gradle-plugin` functional tests when specified.
