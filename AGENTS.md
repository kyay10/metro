...
- `./gradlew :compiler:jvmTest` - Run legacy compiler tests
- `./gradlew :compiler-tests:jvmTest` - Run new compiler tests
...
To create a new test, add a source file under the appropriate directory and then run `./gradlew :compiler-tests:generateTests` to regenerate tests. This will then add a generated junit test that can be run via the standard `./gradlew :compiler-tests:jvmTest` task.
...
2. Test existing tests with `./gradlew :compiler:jvmTest`.
